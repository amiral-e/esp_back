...
category_put.put('/:id', AdminMiddleware, async (c: any) => {
	...
	const { data: categData, error: categError } = await config.supabaseClient.from("categories").select("name").eq("id", id).single();
	if (categData == undefined || categData.length == 0)
		return c.json({ error: "Category not found" }, 404);
	else if (categError != undefined)
		return c.json({ error: categError.message }, 500);
	
	const { data: updateData, error: updateError } = await config.supabaseClient.from("categories").update(json).eq("id", id).select();
	if (updateError != undefined)
		return c.json({ error: updateError.message }, 500);
	return c.json({ response: `Category ${categData.name} updated successfully` }, 200);
});
